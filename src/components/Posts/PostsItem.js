...
export default function PostsItem() {
  ...
  const renderPostsBySearch = () => {
    return postListData.filter(entry => Object.values(entry).some(val => typeof val === "string" && val.match(searchValue))).map((item, index) => {
  ...
  const renderMore = () => {
    if (count >= postListData.length) {
      ...
      return <button className='button__more' onClick={() => setCount(count + 3)}>
        {t('more posts')}
      </button>
    }
  }
